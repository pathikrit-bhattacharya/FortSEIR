...
do i = 1,nac
	C_s(:,i) = w_s(i)*C_s(:,i)   ! Age-group dependent modification at school
	C_w(:,i) = w_w(i)*C_w(:,i)   ! Age-group dependent modification at work
	C_h(:,i) = w_h(i)*C_h(:,i)   ! Age-group dependent modification at home
	C_o(:,i) = w_o(i)*C_o(:,i)   ! Age-group dependent modification at other locations
enddo
...
yt(2:neq:ncmp) = 0.d0            ! Initial population of exposed    - E_a's
yt(3:neq:ncmp) = 0.75*I0(:)      ! Initial proportion of infected, symptomatic   - I_s's
...
yt(6:neq:ncmp) = Nis*N_tot       ! Initial value of population fraction
...
inquire(iolength = irecspace) yt(:)  ! How many machine specific units of data shall we dump on each write?
...
!-----------------------------------------------------------------------------------
    ...
    ! Use Runge-Kutta 4,5 integrator to integrate 
		          ...
		          write(*,*) '# of completed steps = ', i, ' out of ', nt, ' steps'
	  ...
	  write(20,rec=(i-1)+1) yt(:) ! Write out spacefile
enddo
